...
const audioCache = new Map(); // Cache audio to minimize API calls
...
function SpeakButton({ text, voiceName, rate = 0.8, size = "md", color = "#E8734A" }) {
  ...
  return (
    <button onClick={handle} style={{
      ...
      fontSize: icon, flexShrink: 0, animation: state === "loading" ? "pulse 1s infinite" : "none",
    }}>
...
function ProgressBar({ current, total, color = "#E8734A" }) {
  return (
    <div style={{ width: "100%", height: 10, background: "#e8e0d8", borderRadius: 5, overflow: "hidden" }}>
      <div style={{ width: `${total > 0 ? (current / total) * 100 : 0}%`, height: "100%", background: color, borderRadius: 5, transition: "width 0.5s ease" }} />
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
      {Array.from({ length: max }).map((_, i) => (
        <span key={i} style={{ fontSize: 28, filter: i < score ? "none" : "grayscale(1) opacity(0.3)" }}>⭐</span>
      ))}
    </div>
  );
}
  ...
  return (
    <div onClick={onFlip} style={{ cursor: "pointer", width: "100%", maxWidth: 400, minHeight: 240, margin: "0 auto", perspective: 1000 }}>
      <div style={{ width: "100%", minHeight: 240, position: "relative", transformStyle: "preserve-3d", transition: "transform 0.6s cubic-bezier(0.4,0,0.2,1)", transform: flipped ? "rotateY(180deg)" : "rotateY(0)" }}>
        <div style={{ position: "absolute", width: "100%", minHeight: 240, backfaceVisibility: "hidden", background: `linear-gradient(135deg, ${color}, ${color}dd)`, borderRadius: 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, boxShadow: `0 8px 32px ${color}44`, color: "white", boxSizing: "border-box" }}>
          <div style={{ fontSize: 42, fontFamily: "'Baloo 2'", fontWeight: 700 }}>{word.gujarati}</div>
          <div style={{ fontSize: 20, fontFamily: "'Nunito'", fontWeight: 600, marginTop: 8, opacity: 0.9 }}>{word.roman}</div>
          <div style={{ marginTop: 12 }}><SpeakButton text={word.gujarati} voiceName={voiceName} rate={speakRate} size="lg" color="white" /></div>
          <div style={{ fontSize: 12, marginTop: 8, opacity: 0.6 }}>tap card to flip</div>
        </div>
        <div style={{ position: "absolute", width: "100%", minHeight: 240, backfaceVisibility: "hidden", transform: "rotateY(180deg)", background: "white", borderRadius: 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, boxShadow: "0 8px 32px #00000022", boxSizing: "border-box" }}>
          <div style={{ fontSize: 24, fontFamily: "'Baloo 2'", fontWeight: 700, color }}>{word.english}</div>
          <div style={{ marginTop: 12, padding: "6px 14px", background: `${color}15`, borderRadius: 10, fontWeight: 700, fontSize: 16, color, display: "flex", alignItems: "center", gap: 8 }}>
            <SpeakButton text={word.gujarati} voiceName={voiceName} rate={speakRate} size="sm" color={color} />
            {word.pronunciation}
          </div>
          <div style={{ marginTop: 12, fontSize: 13, color: "#6b5e54", textAlign: "center", lineHeight: 1.5, maxWidth: 320 }}>💡 {word.tip}</div>
        </div>
      </div>
    </div>
  );
...
function SettingsPanel({ voiceName, setVoiceName, speakRate, setSpeakRate, onClose, ttsOk }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "#00000066", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div style={{ background: "white", borderRadius: "24px 24px 0 0", padding: "20px 20px 36px", width: "100%", maxWidth: 480, maxHeight: "80vh", overflow: "auto", boxShadow: "0 -16px 48px #00000033" }}>
        <div style={{ width: 40, height: 4, background: "#ddd", borderRadius: 2, margin: "0 auto 16px" }} />
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div style={{ fontFamily: "'Baloo 2'", fontSize: 22, fontWeight: 700 }}>🔊 Audio Settings</div>
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: 24, cursor: "pointer", color: "#8b7d74", padding: 8 }}>✕</button>
        </div>
        <div style={{ padding: "10px 14px", borderRadius: 12, marginBottom: 16, background: ttsOk ? "#E8F5E9" : "#FFF3E6", fontSize: 13, fontWeight: 600, color: ttsOk ? "#2E7D32" : "#E8734A" }}>
          {ttsOk ? "✅ Audio is working! Powered by Google Cloud TTS." : "⏳ Audio not tested yet — tap a speaker button to try."}
        </div>
        <div style={{ marginBottom: 20 }}>
          <label style={{ fontWeight: 700, fontSize: 14, display: "block", marginBottom: 8 }}>Voice</label>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {VOICE_OPTIONS.map(v => (
              <button key={v.name} onClick={() => { setVoiceName(v.name); save("gs_voice", v.name); }} style={{
                padding: "12px 14px", borderRadius: 12, border: voiceName === v.name ? "2px solid #E8734A" : "2px solid #e8e0d8",
                background: voiceName === v.name ? "#E8734A0D" : "white", cursor: "pointer", fontSize: 13, textAlign: "left",
              }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontWeight: 700 }}>{v.label}</span>
                  <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 6, background: v.tier === "wavenet" ? "#4A90D922" : "#e8e0d8", color: v.tier === "wavenet" ? "#4A90D9" : "#8b7d74", fontWeight: 700 }}>{v.tier}</span>
                </div>
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={{ fontWeight: 700, fontSize: 14, display: "block", marginBottom: 6 }}>Speed: {speakRate}x {speakRate <= 0.7 ? "🐢" : speakRate <= 0.9 ? "🚶" : "🏃"}</label>
        ...
        <div style={{ padding: 16, background: "#FFF3E6", borderRadius: 14, textAlign: "center" }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>Test audio</div>
          <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap" }}>
            {["નમસ્તે", "કેમ છો", "ધન્યવાદ"].map(w => (
              <button key={w} onClick={() => speakGujarati(w, voiceName, speakRate)} style={{ padding: "10px 16px", borderRadius: 12, border: "none", background: "#E8734A", color: "white", cursor: "pointer", fontFamily: "'Baloo 2'", fontWeight: 700, fontSize: 14 }}>🔊 {w}</button>
            ))}
          </div>
        </div>
...
export default function App() {
  ...
  // Test TTS on first speaker button success
  useEffect(() => {
    const origSpeak = speakGujarati;
    // We'll set ttsOk when audioCache gets its first entry
  ...
  const navBtn = (label, emoji, target) => (
    <button key={target} onClick={() => setScreen(target)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, padding: "10px 4px", background: screen === target ? "#E8734A" : "transparent", color: screen === target ? "white" : "#8b7d74", border: "none", borderRadius: 14, cursor: "pointer", fontFamily: "'Nunito'", fontWeight: 700, fontSize: 10 }}>
      <span style={{ fontSize: 22 }}>{emoji}</span>{label}
    </button>
  );

  return (
    <div style={{ fontFamily: "'Nunito', sans-serif", background: "#FFF9F2", height: "100%", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      <style>{`
        ...
        input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:24px;height:24px;border-radius:50%;background:#E8734A;cursor:pointer}
      `}</style>
      ...
      <div style={{ background: "linear-gradient(135deg, #E8734A, #D4543A)", padding: "16px 20px 14px", color: "white", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div>
          <div style={{ fontFamily: "'Baloo 2'", fontSize: 24, fontWeight: 800, lineHeight: 1 }}>ગુજરાતી શીખો</div>
          <div style={{ fontSize: 11, opacity: 0.85, fontWeight: 600 }}>Learn Gujarati</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 12, fontWeight: 700 }}>🔥 {streak}</div>
            <div style={{ fontSize: 10, opacity: 0.8 }}>{totalWordsLearned}/{totalWords}</div>
          </div>
          <button onClick={() => setShowSettings(true)} style={{ width: 38, height: 38, borderRadius: 12, border: "2px solid #ffffff44", background: "#ffffff22", color: "white", cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>🔊</button>
        </div>
      </div>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflow: "auto", padding: "16px 16px 90px", WebkitOverflowScrolling: "touch" }}>
        ...
        {screen === "home" && (
          <div>
            <div style={{ background: "linear-gradient(135deg, #FFF3E6, #FFE8D6)", borderRadius: 20, padding: 20, marginBottom: 20, border: "2px solid #F0D4B8" }}>
              <div style={{ fontFamily: "'Baloo 2'", fontSize: 20, fontWeight: 700 }}>Welcome, little learner! 🌟</div>
              <div style={{ fontSize: 13, color: "#6b5e54", marginTop: 6, lineHeight: 1.5 }}>Tap a category to start learning Gujarati words with real audio pronunciation!</div>
              <div style={{ marginTop: 12 }}><ProgressBar current={totalWordsLearned} total={totalWords} /><div style={{ fontSize: 11, color: "#8b7d74", marginTop: 4 }}>{totalWordsLearned} of {totalWords} words explored</div></div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              <button onClick={() => setScreen("sentences")} style={{ padding: 16, background: "white", border: "2px solid #e8e0d8", borderRadius: 16, cursor: "pointer", textAlign: "center" }}><div style={{ fontSize: 28 }}>📝</div><div style={{ fontWeight: 700, fontSize: 13, marginTop: 4 }}>Sentences</div></button>
              <button onClick={() => setScreen("conversations")} style={{ padding: 16, background: "white", border: "2px solid #e8e0d8", borderRadius: 16, cursor: "pointer", textAlign: "center" }}><div style={{ fontSize: 28 }}>💬</div><div style={{ fontWeight: 700, fontSize: 13, marginTop: 4 }}>Conversations</div></button>
            </div>
            <div style={{ fontFamily: "'Baloo 2'", fontSize: 18, fontWeight: 700, marginBottom: 10 }}>Word Categories</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {Object.entries(CATEGORIES).map(([k, c]) => (
                <button key={k} onClick={() => { setSelectedCat(k); setCardIdx(0); setFlipped(false); setScreen("learn"); }} style={{ padding: 16, background: "white", border: `2px solid ${c.color}33`, borderRadius: 16, cursor: "pointer", textAlign: "left", position: "relative", overflow: "hidden" }}>
                  <div style={{ position: "absolute", top: -8, right: -8, fontSize: 48, opacity: 0.1 }}>{c.emoji}</div>
                  <div style={{ fontSize: 24 }}>{c.emoji}</div>
                  <div style={{ fontWeight: 800, fontSize: 14, marginTop: 6 }}>{c.label}</div>
                  <div style={{ fontSize: 11, color: "#8b7d74", marginTop: 2 }}>{c.words.length} words</div>
                  {progress[k]?.learned > 0 && <div style={{ marginTop: 6 }}><ProgressBar current={progress[k].learned} total={c.words.length} color={c.color} /></div>}
                </button>
              ))}
            </div>
          </div>
        ...
        {screen === "learn" && selectedCat && (() => {
          ...
          return (<div>
            ...
            {cardIdx === c.words.length - 1 && <div style={{ textAlign: "center", marginTop: 20 }}><div style={{ fontSize: 14, color: "#6b5e54", marginBottom: 8 }}>🎉 All cards done!</div><button onClick={() => startQuiz(selectedCat)} style={{ padding: "14px 28px", borderRadius: 16, border: "none", background: "linear-gradient(135deg, #4A90D9, #357ABD)", color: "white", cursor: "pointer", fontWeight: 800, fontSize: 15, boxShadow: "0 4px 16px #4A90D944" }}>🧠 Take the Quiz!</button></div>}
          </div>);
        })()}

        {screen === "quiz" && quizState && (() => {
          if (quizState.finished) { const { score } = quizState, t = quizState.questions.length, st = score === t ? 3 : score >= t * 0.6 ? 2 : score > 0 ? 1 : 0; return (<div style={{ textAlign: "center", paddingTop: 40 }}><div style={{ fontSize: 60 }}>{st === 3 ? "🏆" : st >= 2 ? "🌟" : "💪"}</div><div style={{ fontFamily: "'Baloo 2'", fontSize: 28, fontWeight: 800, marginTop: 12 }}>{st === 3 ? "Perfect!" : st >= 2 ? "Great job!" : "Keep practicing!"}</div><div style={{ fontSize: 18, color: "#6b5e54", marginTop: 8 }}>{score}/{t} correct</div><div style={{ marginTop: 12 }}><StarRating score={st} /></div><div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 28 }}><button onClick={() => startQuiz(quizState.catKey)} style={{ padding: "12px 22px", borderRadius: 14, border: "2px solid #E8734A", background: "white", color: "#E8734A", cursor: "pointer", fontWeight: 700, fontSize: 14 }}>Try Again</button><button onClick={() => setScreen("home")} style={{ padding: "12px 22px", borderRadius: 14, border: "none", background: "#E8734A", color: "white", cursor: "pointer", fontWeight: 700, fontSize: 14 }}>Home</button></div></div>); }
          const q = quizState.questions[quizState.current], c = CATEGORIES[quizState.catKey];
          return (<div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><div style={{ fontFamily: "'Baloo 2'", fontSize: 18, fontWeight: 700 }}>Quiz: {c.label} {c.emoji}</div><div style={{ fontSize: 13, color: "#8b7d74", fontWeight: 700 }}>{quizState.current + 1}/{quizState.questions.length}</div></div>
            <ProgressBar current={quizState.current + 1} total={quizState.questions.length} color={c.color} />
            <div style={{ background: "white", borderRadius: 20, padding: 28, marginTop: 20, textAlign: "center", boxShadow: "0 4px 20px #00000008" }}>
              <div style={{ fontSize: 13, color: "#8b7d74", marginBottom: 8 }}>What does this mean?</div>
              <div style={{ fontFamily: "'Baloo 2'", fontSize: 36, fontWeight: 700, color: c.color }}>{q.word.gujarati}</div>
              <div style={{ fontSize: 16, color: "#6b5e54", fontWeight: 600, marginTop: 4 }}>{q.word.roman}</div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 8 }}><SpeakButton text={q.word.gujarati} voiceName={voiceName} rate={speakRate} size="md" color={c.color} /><span style={{ fontSize: 12, color: "#8b7d74" }}>{q.word.pronunciation}</span></div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 20 }}>
              {q.options.map(o => { let bg = "white", bd = "#e8e0d8", tc = "#2d2420"; if (quizState.answered !== null) { if (o === q.correct) { bg = "#E8F5E9"; bd = "#4CAF50"; tc = "#2E7D32"; } else if (o === quizState.answered) { bg = "#FFEBEE"; bd = "#F44336"; tc = "#C62828"; } } return <button key={o} onClick={() => answerQuiz(o)} style={{ padding: 16, borderRadius: 14, border: `2px solid ${bd}`, background: bg, cursor: quizState.answered ? "default" : "pointer", fontWeight: 700, fontSize: 15, color: tc }}>{o}</button>; })}
            </div>
            {quizState.answered !== null && <div style={{ textAlign: "center", marginTop: 16 }}><div style={{ fontSize: 14, fontWeight: 700, marginBottom: 10, color: quizState.answered === q.correct ? "#2E7D32" : "#C62828" }}>{quizState.answered === q.correct ? "✅ Correct!" : `❌ Answer: "${q.correct}"`}</div><button onClick={nextQ} style={{ padding: "12px 28px", borderRadius: 14, border: "none", background: c.color, color: "white", cursor: "pointer", fontWeight: 700, fontSize: 14 }}>{quizState.current + 1 < quizState.questions.length ? "Next →" : "Results 🎉"}</button></div>}
          </div>);
        })()}

        {screen === "sentences" && (() => { const s = SENTENCES[sentenceIdx], tt = s.ttsText || s.gujarati; return (<div>
          <button onClick={() => setScreen("home")} style={{ background: "none", border: "none", cursor: "pointer", fontWeight: 700, color: "#8b7d74", fontSize: 14, marginBottom: 12, padding: 0 }}>← Back</button>
          <div style={{ fontFamily: "'Baloo 2'", fontSize: 22, fontWeight: 700, marginBottom: 4 }}>📝 Sentences</div>
          <ProgressBar current={sentenceIdx + 1} total={SENTENCES.length} color="#9B59B6" /><div style={{ fontSize: 11, color: "#8b7d74", marginTop: 4, marginBottom: 16 }}>{sentenceIdx + 1}/{SENTENCES.length}</div>
          <div style={{ background: "white", borderRadius: 20, padding: 24, boxShadow: "0 4px 20px #00000008" }}>
            <div style={{ fontFamily: "'Baloo 2'", fontSize: 26, fontWeight: 700, color: "#9B59B6", textAlign: "center" }}>{s.gujarati}</div>
            <div style={{ textAlign: "center", fontSize: 15, color: "#6b5e54", fontWeight: 600, marginTop: 6 }}>{s.roman}</div>
            <div style={{ display: "flex", justifyContent: "center", marginTop: 10 }}><SpeakButton text={tt} voiceName={voiceName} rate={speakRate} size="lg" color="#9B59B6" /></div>
            <div style={{ textAlign: "center", marginTop: 12, padding: "10px 16px", background: "#F3E5F5", borderRadius: 12 }}><div style={{ fontSize: 17, fontWeight: 700 }}>{s.english}</div></div>
            <div style={{ marginTop: 14, padding: "10px 14px", background: "#FFF8E1", borderRadius: 12 }}><div style={{ fontSize: 14, fontWeight: 700, color: "#E8734A" }}>{s.pronunciation}</div><div style={{ fontSize: 12, color: "#6b5e54", marginTop: 4, lineHeight: 1.4 }}>💡 {s.tip}</div></div>
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "center" }}>
            <button disabled={sentenceIdx === 0} onClick={() => setSentenceIdx(i => i - 1)} style={{ padding: "12px 24px", borderRadius: 14, border: "2px solid #e8e0d8", background: "white", cursor: sentenceIdx === 0 ? "default" : "pointer", fontWeight: 700, fontSize: 14, opacity: sentenceIdx === 0 ? 0.4 : 1 }}>← Prev</button>
            <button disabled={sentenceIdx === SENTENCES.length - 1} onClick={() => setSentenceIdx(i => i + 1)} style={{ padding: "12px 24px", borderRadius: 14, border: "none", background: "#9B59B6", color: "white", cursor: sentenceIdx === SENTENCES.length - 1 ? "default" : "pointer", fontWeight: 700, fontSize: 14, opacity: sentenceIdx === SENTENCES.length - 1 ? 0.5 : 1 }}>Next →</button>
          </div>
        </div>); })()}

        {screen === "conversations" && (() => { const c = CONVERSATIONS[convoIdx], ln = c.lines[convoLine]; return (<div>
          <button onClick={() => setScreen("home")} style={{ background: "none", border: "none", cursor: "pointer", fontWeight: 700, color: "#8b7d74", fontSize: 14, marginBottom: 12, padding: 0 }}>← Back</button>
          <div style={{ fontFamily: "'Baloo 2'", fontSize: 22, fontWeight: 700, marginBottom: 4 }}>💬 Conversations</div>
          <div style={{ display: "flex", gap: 8, marginBottom: 16, overflowX: "auto", paddingBottom: 4 }}>
            {CONVERSATIONS.map((cv, i) => <button key={i} onClick={() => { setConvoIdx(i); setConvoLine(0); setConvoRevealed(false); }} style={{ padding: "8px 14px", borderRadius: 12, border: "none", background: i === convoIdx ? "#E8734A" : "#e8e0d8", color: i === convoIdx ? "white" : "#6b5e54", cursor: "pointer", fontWeight: 700, fontSize: 12, whiteSpace: "nowrap" }}>{cv.emoji} {cv.title}</button>)}
          </div>
          <ProgressBar current={convoLine + 1} total={c.lines.length} color="#E8734A" /><div style={{ fontSize: 11, color: "#8b7d74", marginTop: 4, marginBottom: 16 }}>Line {convoLine + 1}/{c.lines.length}</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {c.lines.slice(0, convoLine).map((l, i) => { const left = i % 2 === 0; return (
              <div key={i} style={{ display: "flex", justifyContent: left ? "flex-start" : "flex-end", alignItems: "flex-start", gap: 6 }}>
                ...
                <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: 16, background: left ? "#e8e0d8" : "#E8734A22", borderBottomLeftRadius: left ? 4 : 16, borderBottomRightRadius: left ? 16 : 4 }}><div style={{ fontSize: 10, fontWeight: 700, color: "#8b7d74" }}>{l.speaker}</div><div style={{ fontSize: 14, fontWeight: 600 }}>{l.gujarati}</div><div style={{ fontSize: 11, color: "#8b7d74" }}>{l.english}</div></div>
                {!left && <SpeakButton text={l.gujarati} voiceName={voiceName} rate={speakRate} size="sm" color="#8b7d74" />}
              </div>
            ); })}
            <div style={{ background: "white", borderRadius: 20, padding: 20, boxShadow: "0 4px 20px #00000010", border: "2px solid #E8734A33" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}><div style={{ fontSize: 11, fontWeight: 700, color: "#E8734A" }}>{ln.speaker} says:</div><SpeakButton text={ln.gujarati} voiceName={voiceName} rate={speakRate} size="md" color="#E8734A" /></div>
              <div style={{ fontFamily: "'Baloo 2'", fontSize: 22, fontWeight: 700, marginTop: 4 }}>{ln.gujarati}</div>
              <div style={{ fontSize: 14, color: "#6b5e54", fontWeight: 600, marginTop: 4 }}>{ln.roman}</div>
              {!convoRevealed ? <button onClick={() => setConvoRevealed(true)} style={{ marginTop: 12, padding: "10px 20px", borderRadius: 12, border: "2px dashed #E8734A", background: "transparent", color: "#E8734A", cursor: "pointer", fontWeight: 700, fontSize: 13 }}>🤔 Show meaning</button> : <div style={{ marginTop: 12 }}><div style={{ padding: "8px 12px", background: "#FFF3E6", borderRadius: 10, marginBottom: 8 }}><div style={{ fontSize: 15, fontWeight: 700 }}>{ln.english}</div></div><div style={{ padding: "8px 12px", background: "#FFF8E1", borderRadius: 10 }}><div style={{ fontSize: 13, fontWeight: 700, color: "#E8734A" }}>💡 {ln.pronunciation}</div></div></div>}
            </div>
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "center" }}>
            <button disabled={convoLine === 0} onClick={() => { setConvoLine(i => i - 1); setConvoRevealed(false); }} style={{ padding: "12px 24px", borderRadius: 14, border: "2px solid #e8e0d8", background: "white", cursor: convoLine === 0 ? "default" : "pointer", fontWeight: 700, fontSize: 14, opacity: convoLine === 0 ? 0.4 : 1 }}>← Prev</button>
            <button disabled={convoLine === c.lines.length - 1} onClick={() => { setConvoLine(i => i + 1); setConvoRevealed(false); }} style={{ padding: "12px 24px", borderRadius: 14, border: "none", background: "#E8734A", color: "white", cursor: convoLine === c.lines.length - 1 ? "default" : "pointer", fontWeight: 700, fontSize: 14, opacity: convoLine === c.lines.length - 1 ? 0.5 : 1 }}>Next →</button>
          </div>
        </div>); })()}

        {screen === "alphabet" && (<div>
          <div style={{ fontFamily: "'Baloo 2'", fontSize: 22, fontWeight: 700, marginBottom: 4 }}>🔤 Gujarati Sounds</div>
          <div style={{ fontSize: 12, color: "#8b7d74", marginBottom: 6, lineHeight: 1.4 }}>Aspirated consonants (ખ, ઘ, છ) use a <strong>puff of air</strong>. Hold your hand near your mouth to feel it!</div>
          <div style={{ fontSize: 11, padding: "6px 10px", background: "#FFF3E6", borderRadius: 8, marginBottom: 14, color: "#6b5e54" }}><strong>Tip:</strong> <em>Dental</em> = tongue on teeth (ત, દ). <em>Retroflex</em> = tongue curled back (ટ, ડ).</div>
          <ProgressBar current={alphabetPage + 1} total={alphaPages} color="#4A90D9" /><div style={{ fontSize: 11, color: "#8b7d74", marginTop: 4, marginBottom: 12 }}>Page {alphabetPage + 1}/{alphaPages}</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {PRONUNCIATION_GUIDE.slice(alphabetPage * ALPHA_PP, (alphabetPage + 1) * ALPHA_PP).map((it, i) => (
              <div key={i} style={{ background: "white", borderRadius: 14, padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 2px 8px #00000006" }}>
                <div style={{ width: 46, height: 46, borderRadius: 12, background: "linear-gradient(135deg, #4A90D9, #357ABD)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontFamily: "'Baloo 2'", fontSize: 22, fontWeight: 700, flexShrink: 0 }}>{it.letter}</div>
                <div style={{ flex: 1 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontWeight: 800, fontSize: 15 }}>{it.roman}</span><span style={{ fontSize: 12, color: "#E8734A", fontWeight: 700 }}>/{it.sound}/</span></div><div style={{ fontSize: 11, color: "#6b5e54", marginTop: 2 }}>{it.example}</div></div>
                <SpeakButton text={it.letter} voiceName={voiceName} rate={0.6} size="sm" color="#4A90D9" />
              </div>
            ))}
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "center" }}>
            <button disabled={alphabetPage === 0} onClick={() => setAlphabetPage(p => p - 1)} style={{ padding: "12px 24px", borderRadius: 14, border: "2px solid #e8e0d8", background: "white", cursor: alphabetPage === 0 ? "default" : "pointer", fontWeight: 700, fontSize: 14, opacity: alphabetPage === 0 ? 0.4 : 1 }}>← Prev</button>
            <button disabled={alphabetPage === alphaPages - 1} onClick={() => setAlphabetPage(p => p + 1)} style={{ padding: "12px 24px", borderRadius: 14, border: "none", background: "#4A90D9", color: "white", cursor: alphabetPage === alphaPages - 1 ? "default" : "pointer", fontWeight: 700, fontSize: 14, opacity: alphabetPage === alphaPages - 1 ? 0.5 : 1 }}>Next →</button>
          </div>
        </div>)}
      </div>

      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "white", borderTop: "1px solid #e8e0d8", display: "flex", padding: "6px 8px", paddingBottom: "max(6px, env(safe-area-inset-bottom))", gap: 4, boxSizing: "border-box", zIndex: 100 }}>
        {navBtn("Home", "🏠", "home")}
